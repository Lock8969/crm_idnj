...
if (typeof window.saveVehicleInfo !== 'function') {
    window.saveVehicleInfo = function(clientId) {
        ...
        .then(data => {
            if (data.success) {
                ...
                // Optionally redirect to client detail page
...
/**
 * Render vehicle info modal via JavaScript
 * This function generates JavaScript code to create the modal dynamically
 * 
 * @param int $clientId The client ID
 * @return string JavaScript code to create the modal
 */
function renderVehicleInfoModalJS($clientId) {
    ob_start();
?>
<script>
// Create vehicle info modal for client ID: <?php echo $clientId; ?>
(function() {
    // Create modal HTML
    const vehicleModalHtml = `
    <div class="modal fade" id="vehicleInfoModal<?php echo $clientId; ?>" tabindex="-1" aria-labelledby="vehicleInfoModalLabel<?php echo $clientId; ?>" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
    ...
    if (!document.getElementById('vehicleInfoModal<?php echo $clientId; ?>')) {
        document.body.insertAdjacentHTML('beforeend', vehicleModalHtml);
    }
    
    // Define saveVehicleInfo function if it doesn't exist
    if (typeof window.saveVehicleInfo !== 'function') {
        window.saveVehicleInfo = function(clientId) {
            const form = document.getElementById('vehicleInfoForm' + clientId);
            const formData = new FormData(form);
